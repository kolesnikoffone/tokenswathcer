...
async def ignore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        ...
        ignored_tokens.add(address)
        await update.message.reply_text(f"✅ Токен с адресом {address} добавлен в игнор.")
    else:
        await update.message.reply_text("❗ Укажи адрес токена: /ignore <address>")

async def deignore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        address = context.args[0]
        if address in ignored_tokens:
            ignored_tokens.remove(address)
            await update.message.reply_text(f"♻️ Токен с адресом {address} убран из игнора.")
        else:
            await update.message.reply_text(f"ℹ️ Токен {address} не был в игноре.")
    else:
        await update.message.reply_text("❗ Укажи адрес токена: /deignore <address>")

async def ignorelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if ignored_tokens:
        msg = "📛 Игнорируемые токены:\n" + "\n".join(ignored_tokens)
    else:
        msg = "✅ Игнор-лист пуст."
